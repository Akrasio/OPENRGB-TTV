const { Client, utils } = require("openrgb-sdk"); // for your usecase use require("openrgb-sdk")
const ms = 200;

async function random() {
	// initiate a client and connect to it
	const client = new Client("OpenrgbTTV_NodeJS", 6742, "localhost");
...
async function color(hex) {
	...
	async function loop() {
		for (let deviceId = 0; deviceId < controllerCount; deviceId++) {
			const { colors } = await client.getControllerData(deviceId);
			await client.updateLeds(deviceId, get_color(colors.length))
		}
		client.disconnect()
	}
	// start the loop
